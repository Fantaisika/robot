...
/*-----------------------------meteoAgricole-----------------------------*/
const meteoAgricoleSchema = new Schema({
    coord: {
        lon: Number,
        lat: Number,
    },
...
const METEOAGRICOLE = mongoose.model('METEOAGRICOLE', meteoAgricoleSchema);
...
async function fetchAndSaveWeatherMeteoAgricole(cities) {
    ...
    for (const city of cities) {
        ...
        try {
            ...
            if (data.cod === 200) {
                ...
                let newMeteoAgricoleData = new METEOAGRICOLE({
                    coord: data.coord,
                ...
                await newMeteoAgricoleData.save();
                console.log(`Données météo pour ${city} enregistrées avec succès :`, data);
            } else {
...
// Appeler la fonction pour chaque département et ses villes associées
for (const department in citiesByDepartment) {
    const cities = citiesByDepartment[department];
    fetchAndSaveWeatherMeteoAgricole([department, ...cities]);
}
